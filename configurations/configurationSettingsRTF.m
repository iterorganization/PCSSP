function cs = configurationSettingsRTF()
% MATLAB function for configuration set generated on 12-May-2023 14:39:24
...
cs.set_param('Name', 'configurationSettingsRTF'); % Name
...
cs.set_param('StopTime', '40');   % Stop time
...
cs.set_param('PostCodeGenCommand', 'packNGo(buildInfo);');   % Post code generation command
...
cs.set_param('CustomSourceCode', sprintf('%s\n%s\n%s\n%s','void *GETMMIFCN(MODEL)(void *rtm)','{','  return (void *)&((static_cast<TSTRUCTNAME(MODEL) *>(rtm)->DataMapInfo).mmi);','}'));   % Source file
cs.set_param('CustomHeaderCode', sprintf('%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s','#ifndef SCDMACROS','#define SCDMACROS','  #define UTSTRUCTNAME(NAME)             RT_MODEL_##NAME##_T','  #define TSTRUCTNAME(NAME)              UTSTRUCTNAME(NAME)','  #define UGETMMIFCN(NAME)               NAME##_GetCAPImmi','  #define GETMMIFCN(NAME)                UGETMMIFCN(NAME)','#endif','','#ifdef __cplusplus','','extern "C" {','','#endif','','  void *GETMMIFCN(MODEL)(void *rtm);','','#ifdef __cplusplus','','}','#endif'));   % Header file
...
cs.set_param('Toolchain', 'GNU gcc/g++ | gmake (64-bit Linux)');   % Toolchain
...
cs.set_param('GlobalDataDefinition', 'InSeparateSourceFile');   % Data definition
cs.set_param('GlobalDataReference', 'InSeparateHeaderFile');   % Data declaration
...
cs.set_param('DataDefinitionFile', 'global.c');   % Data definition filename
cs.set_param('DataReferenceFile', 'global.h');   % Data declaration filename
